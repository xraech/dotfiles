...
function helpers.volume_control(step)
    ...
    if step == 0 then
        ...
        awful.spawn.easy_async_with_shell(cmd, function(out)
            ...
            print(out)
            if not sidebar.visible then
                if self.volume_notif and not self.volume_notif.is_expired then
					self.volume_notif.message = out
                else
                    self.volume_notif = naughty.notification({ title = "Volume", message = out, timeout = 2 })
                end
            end
